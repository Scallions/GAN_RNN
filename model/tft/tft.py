...
class TFT(nn.Layer):
    def __init__(self, c_in=35, c_out=35, encode_length=384, seq_length=480, hidden_size=160, dropout=0.0, attn_heads=4, quantiles_num=3):
        super().__init__()
        # params = dict(raw_params)  # copy locally
        # print(params)

        ## data params
        self.input_size = c_in+5
        self.output_size = c_out

        self.input_obs_loc = list(range(c_in+4))
        self.static_loc = [c_in+4]
        self.static_variables = len(self.static_loc)
        self.cat_counts = [c_in]
        self.know_reg = list(map(lambda x:x+c_in, [0,1,2,3]))
        self.know_cat = [c_in+4]

        # network params
        self.hidden_size = hidden_size
        self.dropout = dropout
        self.attn_heads = attn_heads
        self.num_quantiles = quantiles_num
        ...
        self.history_select = TimeVariableSelectionNetwork(self.encode_length, self.hidden_size, 43, self.hidden_size,
                                                           self.dropout, self.static_variables * self.hidden_size)
        self.future_select = TimeVariableSelectionNetwork(self.seq_length - self.encode_length, self.hidden_size, 4,
                                                          self.hidden_size, self.dropout,
        ...
        self.out_layer = Linear(self.hidden_size, c_out, use_td=True)
    ...
    def forward(self, x):
        ...
        # emb_cat_inps = paddle.stack(emb_cat_inps, axis=1)
        ### sta emb
        sta_inps = [self.static_embedding_layers[i](inputs[:, 0, i]) for i in range(num_reg) if
